...
from abletoolz.classes.ableton_track import AbletonTrack
from abletoolz.common import RST, R, G, B, Y, C
from abletoolz.common import print_rst, get_element, BACKUP_DIR, STEREO_OUTPUTS
...
class AbletonSet(object):
    ...
    def parse(self):
        ...
        try:
            with open(self.pathlib_obj, 'rb') as fd:
                ...
                if first_two_bytes == b'\0xab\0x1e':  # yes, it spells able :P
                    print_rst(f'{R}{self.pathlib_obj} Is an older version set, currently not supported.')
                    raise Exception()
                    return False
                elif first_two_bytes != b'\x1f\x8b':
                    print_rst(f'{R}{self.pathlib_obj} File format is not gzip!, cannot open.')
                    raise Exception()
                    return False
            ...
            print_rst(f'{R}Error loading file: {e}')
            return False
    ...
    def move_original_file_to_backup_dir(pathlib_obj):
        ...
        while True:
            ...
            if pathlib.Path(backup_path).exists():
                ...
                print_rst(f'{B}Moving original file to backup directory: {pathlib_obj} --> {backup_path}')
    ...
    def find_project_root_folder(self):
        ...
        for i, current_dir in enumerate(self.pathlib_obj.parents):
            if i > max_folder_search_depth:
                print_rst(f'{R}Reached maximum search depth, exiting..')
                return
            elif pathlib.Path(current_dir / "Ableton Project Info").exists():
                self.project_root_folder = current_dir
                print_rst(f'{G}Project root folder: {current_dir}')
                return
        print_rst(f'{R}Did not find project root folder, relative paths will not be verifiable.')
    ...
    def save_xml(self):
        ...
        print_rst(f'{G}Saved xml to {xml_file}')
    ...
    def save_set(self):
        ...
        print_rst(f'{G}Saved set to {self.pathlib_obj}')

    # Data parsing functions.
    def load_tracks(self):
        """Load tracks into AbletonTrack classes."""
        tracks = get_element(self.root, 'LiveSet.Tracks')
        for track in tracks:
            self.tracks.append(AbletonTrack(track))

    def print_rst_tracks(self):
        """Prints track info."""
    ...
    def get_bpm(self):
        ...
        bpm_elem = get_element(self.root, 'LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual', attribute='Value')
    ...
    def set_bpm(self, bpm):
        """Sets bpm."""
        bpm = round(bpm, 6)
        bpm_elem = get_element(self.root, 'LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual')

        # Only on live 10 sets
        bpm_automation_events = get_element(self.root, 'LiveSet.MasterTrack.AutomationEnvelopes.Envelopes')

        initial_automation_event = get_element(bpm_automation_events[1], 'Automation.Events.FloatEvent')
        bpm_elem.set('Value', str(float(bpm)))
        initial_automation_event.set('Value', str(bpm))
        print_rst(f'{G}Set bpm to {bpm}')
    ...
    def estimate_length(self):
        ...
        return f"{int(seconds_total // 60)}:{round(seconds_total % 60):02d}"
    ...
    def set_track_heights(self, height):
        ...
        print_rst(f'{G}Set track heights to {height}.')
    ...
    def set_audio_output(self, output_number, element_string):
        ...
        print_rst(f"{G}Set {element_string} to {output_obj['lower_display_string']}")
    ...
    def _parse_mac_data(byte_data, abs_hash_path, debug=False):
        ...
        while i <= last_possible_index:
            ...
            if (potential_end < last_possible_index
                ...
                if debug:
                    print_rst(f'\t{Y}i: {i}, value at i: {byte_data[i]}, potential (e)ndex: {potential_end}, '
    ...
    def list_plugins(self):
        ...
        for plugin_element in self.root.iter('PluginDesc'):
            if plugin_element.findall('./VstPluginInfo'):
                ...
                if not path:
                    print_rst(f"{Y}Couldn't parse absolute path for {name}")
                    continue
                ...
                print_rst(f'{color}Plugin: {name}, {Y}Plugin folder path: {full_path}, {color}Exists: {exists}')
                ...
                print_rst(f"{Y}Audio Units do not store paths. Plugin {manufacturer}: {name} cannot be verified.")
    ...
    def check_relative_path(self, name, sample_element):
        ...
        if not self.project_root_folder:
            print_rst(f'{R}Error, project root folder is undefined, cant find relative paths.')
            return
    ...
    def list_samples(self):
        ...
        for sample_element in self.root.iter('SampleRef'):
            ...
            print_rst(f'{C}Sample name: {name}')
            ...
            if relative_path:
                ...
                print_rst(f'\t{color}Relative path: {Y}{relative_path}, {color}Exists: {rel_exists}')
            ...
            print_rst(f'\t{color}Absolute path: {Y}{abs_path}, {color}Exists: {abs_exists}')
